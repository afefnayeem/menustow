...
enum MouseHelpers {
    private static var hideDepth = 0
    private static let hideLock = NSLock()
    ...
    /// Forces the cursor to be visible, clearing the local hide depth.
    static func forceShowCursor() {
        hideLock.lock()
        hideDepth = 0
        hideLock.unlock()
