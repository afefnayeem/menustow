    ...
    private func migrateControlItems0_10_1() -> MigrationResult {
        ...
        if needsResetPreferredPositions {
            ...
            alert.messageText = """
                ...
                menustow’s menu bar sections was corrupted and had to be reset.
                """
...
extension MigrationManager {
    /// Performs all migrations for the `0.12.0` release.
    private func migrate0_12_0() -> MigrationResult {
        guard !Defaults.bool(forKey: .hasMigrated0_12_0) else {
            return .success
        }

        let mappings: [(String, Defaults.Key)] = [
            ("ShowIceIcon", .showMenustowIcon),
            ("IceIcon", .menustowIcon),
            ("CustomIceIconIsTemplate", .customMenustowIconIsTemplate),
            ("UseIceBar", .useMenustowBar),
            ("IceBarLocation", .menustowBarLocation),
        ]

        for (oldKey, newKey) in mappings {
            migrateDefaultsKey(oldKey: oldKey, newKey: newKey)
        }

        migrateHotkeyKey(oldKey: "EnableIceBar", newKey: "EnableMenustowBar")

        Defaults.set(true, forKey: .hasMigrated0_12_0)
        logger.info("Successfully migrated to 0.12.0 settings")
        return .success
    }

    private func migrateDefaultsKey(oldKey: String, newKey: Defaults.Key) {
        let defaults = UserDefaults.standard
        if defaults.object(forKey: newKey.rawValue) == nil,
           let value = defaults.object(forKey: oldKey)
        {
            defaults.set(value, forKey: newKey.rawValue)
        }
        defaults.removeObject(forKey: oldKey)
    }

    private func migrateHotkeyKey(oldKey: String, newKey: String) {
        guard var dictionary = Defaults.dictionary(forKey: .hotkeys) as? [String: Data] else {
            return
        }
        if dictionary[newKey] == nil, let data = dictionary[oldKey] {
            dictionary[newKey] = data
        }
