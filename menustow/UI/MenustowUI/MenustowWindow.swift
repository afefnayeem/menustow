...
/// A custom scene representing one of menustow's windows.
struct MenustowWindow<Content: View>: Scene {
    @Environment(\.openWindow) private var openWindow
    @Environment(\.dismissWindow) private var dismissWindow

    /// The window's identifier.
    let id: MenustowWindowIdentifier
    ...
    init(id: MenustowWindowIdentifier, @ViewBuilder content: () -> Content) {
        self.id = id
...
// MARK: - MenustowWindowIdentifier

/// Custom identifier constants uses to create menustow's windows.
enum MenustowWindowIdentifier: String, Sendable, CustomStringConvertible {
    /// The identifier for menustow's main settings window.
    case settings = "SettingsWindow"

    /// The identifier for menustow's permissions window.
...
extension OpenWindowAction {
    ...
    ///
    /// - Parameter id: An identifier for one of menustow's windows.
    func callAsFunction(id: MenustowWindowIdentifier) {
