...
// MARK: - MenustowGradient

/// A custom gradient.
struct MenustowGradient: Codable, Hashable {
    ...
    func withAlpha(_ alpha: CGFloat) -> MenustowGradient {
        let newStops = stops.map { $0.withAlpha(alpha) }
        return MenustowGradient(stops: newStops)
    }
...
// MARK: MenustowGradient Static Members
extension MenustowGradient {
    /// The default menu bar tint gradient.
    static let defaultMenuBarTint = MenustowGradient(stops: [
        ColorStop.white(location: 0),
        ColorStop.black(location: 1),
    ])
}

// MARK: - MenustowGradient.ColorStop
...
// MARK: MenustowGradient.ColorStop: Codable
extension MenustowGradient.ColorStop: Codable {
    ...
    init(from decoder: any Decoder) throws {
        ...
        self.color = try container.decode(MenustowColor.self, forKey: .color).cgColor
        self.location = try container.decode(CGFloat.self, forKey: .location)
    }

    func encode(to encoder: any Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encode(MenustowColor(cgColor: color), forKey: .color)
