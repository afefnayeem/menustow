...
extension HIDEventManager {
    ...
    private func handleSmartRehide(with event: NSEvent, appState: AppState, screen: NSScreen) {
        ...
        // Make sure clicking the menustow icon doesn't trigger rehide.
        if let menustowIcon = appState.menuBarManager.controlItem(withName: .visible) {
            guard event.window !== menustowIcon.window else {
                return
            }
        }

        // Only continue if the click is not inside the menustow Bar, at
        // least one section is visible, and the mouse is not inside
        // the menu bar.
        guard
            event.window !== appState.menuBarManager.menustowBarPanel,
    ...
    private func handleShowOnHover(appState: AppState, screen: NSScreen) {
        ...
        if hiddenSection.isHidden {
            ...
            guard
                !isMouseInsideMenuBar(appState: appState, screen: screen),
                !isMouseInsideMenustowBar(appState: appState)
            else {
                return
            }
            Task {
    ...
    private func handlePreventShowOnHover(with event: NSEvent, appState: AppState, screen: NSScreen) {
        guard
            appState.settings.general.showOnHover,
            !appState.settings.general.useMenustowBar
        else {
            return
        }
        ...
        if isMouseInsideMenuBarItem(appState: appState, screen: screen) {
            ...
            case .leftMouseDown:
                ...
                if isMouseInsideMenustowIcon(appState: appState) {
                    break
                }
                return
            case .rightMouseDown:
...
extension HIDEventManager {
    ...
    func isMouseInsideMenuBar(appState: AppState, screen: NSScreen) -> Bool {
        // menustow icon must be vertically visible. Otherwise, we can infer
        // that the menu bar is hidden and the mouse is not inside.
        guard
            let menustowIcon = appState.menuBarManager.controlItem(withName: .visible),
            let menustowIconFrame = menustowIcon.frame,
            menustowIconFrame.maxY <= screen.frame.maxY,
            let mouseLocation = MouseHelpers.locationAppKit
        else {
            return false
        }
    ...
    /// A Boolean value that indicates whether the mouse pointer is within
    /// the bounds of the menustow Bar panel.
    func isMouseInsideMenustowBar(appState: AppState) -> Bool {
        guard let mouseLocation = MouseHelpers.locationAppKit else {
            return false
        }
        let panel = appState.menuBarManager.menustowBarPanel
        // Pad the frame to be more forgiving if the user accidentally
        // moves their mouse outside of the menustow Bar.
        let paddedFrame = panel.frame.insetBy(dx: -15, dy: -15)
        return paddedFrame.contains(mouseLocation)
    }

    /// A Boolean value that indicates whether the mouse pointer is within
    /// the bounds of the menustow icon.
    func isMouseInsideMenustowIcon(appState: AppState) -> Bool {
        guard
            let visibleSection = appState.menuBarManager.section(withName: .visible),
            let menustowIconFrame = visibleSection.controlItem.frame,
            let mouseLocation = MouseHelpers.locationAppKit
        else {
            return false
        }
        return menustowIconFrame.contains(mouseLocation)
    }
}
