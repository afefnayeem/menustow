...
final class MenuBarSection {
    ...
    /// A Boolean value that indicates whether the menustow Bar should be used.
    private var useMenustowBar: Bool {
        appState?.settings.general.useMenustowBar ?? false
    }
    ...
    /// The best screen to show the menustow Bar on.
    private weak var screenForMenustowBar: NSScreen? {
    ...
    var isHidden: Bool {
        if useMenustowBar {
            ...
            switch name {
            case .visible, .hidden:
                return menuBarManager?.menustowBarPanel.currentSection != .hidden
            case .alwaysHidden:
                return menuBarManager?.menustowBarPanel.currentSection != .alwaysHidden
            }
        }
        switch name {
        case .visible, .hidden:
            if menuBarManager?.menustowBarPanel.currentSection == .hidden {
                return false
            }
            return controlItem.state == .hideSection
        case .alwaysHidden:
            if menuBarManager?.menustowBarPanel.currentSection == .alwaysHidden {
                return false
            }
            return controlItem.state == .hideSection
        }
    ...
    func show() {
        ...
        if useMenustowBar {
            ...
            // Still update the visible control item (menustow icon) state to show
            // its alternate icon.
            ...
            if let screen = screenForMenustowBar {
                Task {
                    switch name {
                    case .visible, .hidden:
                        await menuBarManager.menustowBarPanel.show(section: .hidden, on: screen)
                    case .alwaysHidden:
                        await menuBarManager.menustowBarPanel.show(section: .alwaysHidden, on: screen)
                    }
                    startRehideChecks()
                }
            }
        ...
        // If we made it here, we're not using the menustow Bar.
        // Make sure it's closed.
        menuBarManager.menustowBarPanel.close()
    ...
    func hide() {
        ...
        menuBarManager.menustowBarPanel.close() // Make sure menustow Bar is always closed.
        menuBarManager.showOnHoverAllowed = true

        switch name {
        case _ where useMenustowBar, .visible, .hidden:
            for section in menuBarManager.sections {
                section.controlItem.state = .hideSection
            }
        case .alwaysHidden:
