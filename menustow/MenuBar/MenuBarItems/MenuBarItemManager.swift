...
final class MenuBarItemManager: ObservableObject {
    ...
    /// Returns a mapping of control item window identifiers to their tags.
    private func controlItemWindowIDMap() -> [CGWindowID: MenuBarItemTag] {
        guard let appState else {
            return [:]
        }
        return appState.menuBarManager.sections.reduce(into: [:]) { result, section in
            guard let windowNumber = section.controlItem.window?.windowNumber else {
                return
            }
            guard
                windowNumber > 0,
            ...
            result[windowID] = section.controlItem.identifier.tag
        }
    }

    /// Re-tags control items based on live window identifiers or frames when possible.
    private func retagControlItems(in items: [MenuBarItem]) -> [MenuBarItem] {
        guard let appState else {
            return items
        }

        var updated = items
        let windowMap = controlItemWindowIDMap()

        if !windowMap.isEmpty {
            updated = updated.map { item in
                guard let tag = windowMap[item.windowID] else {
                    return item
                }
                if item.tag == tag {
                    return item
                }
                return item.replacingTag(tag)
            }
        }

        var presentControlTags = Set(updated.filter { $0.isControlItem }.map(\.tag))
        let controlItems = appState.menuBarManager.sections.map(\.controlItem)

        for controlItem in controlItems {
            let tag = controlItem.identifier.tag
            guard
                !presentControlTags.contains(tag),
                let frame = controlItem.onScreenFrame
            else {
                continue
            }

            if let index = updated.firstIndex(where: { $0.bounds.center.distance(to: frame.center) <= 1 }) {
                updated[index] = updated[index].replacingTag(tag)
                presentControlTags.insert(tag)
            }
        }
...
extension MenuBarItemManager {
    ...
    func cacheItemsRegardless(
        ...
        await cacheActor.runCacheTask { [weak self] in
            ...
            guard bypassMoveBuffer || !lastMoveOperationOccurred(within: .seconds(1)) else {
            ...
            items = self.retagControlItems(in: items)

            let itemWindowIDs = currentItemWindowIDs ?? items.map(\.windowID)
    ...
    func cacheItemsIfNeeded() async {
        let itemWindowIDs = MenuBarItem.getMenuBarItemWindows(option: .activeSpace)
            .map(\.windowID)
...
extension MenuBarItemManager {
    ...
    private nonisolated func getEventPID(for item: MenuBarItem) -> pid_t {
        return item.sourcePID ?? item.ownerPID
    }
...
extension MenuBarItemManager {
    ...
    /// Refreshes the item cache after a move operation.
    private func refreshCacheAfterMove() async {
        // Allow the system a moment to apply the new ordering.
        try? await Task.sleep(for: .milliseconds(150))
        await cacheItemsRegardless(bypassMoveBuffer: true)
    }
