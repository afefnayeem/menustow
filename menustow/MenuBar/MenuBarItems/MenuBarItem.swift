...
struct MenuBarItem: CustomStringConvertible {
    ...
    /// Creates a menu bar item with explicit values.
    init(
        tag: MenuBarItemTag,
        windowID: CGWindowID,
        ownerPID: pid_t,
    ...
    /// A Boolean value that indicates whether this item is one of menustow's
    /// control items.
    ...
    /// Returns a copy of this item with a new tag.
    func replacingTag(_ tag: MenuBarItemTag) -> MenuBarItem {
        MenuBarItem(
            tag: tag,
            windowID: windowID,
            ownerPID: ownerPID,
...
extension MenuBarItem {
    ...
    static func getMenuBarItemWindows(on display: CGDirectDisplayID? = nil, option: ListOption) -> [WindowInfo] {
        ...
        let windows: [WindowInfo] = Bridging.getMenuBarWindowList(option: bridgingOption)
            .compactMap { windowID -> WindowInfo? in
                guard
...
extension MenuBarItemTag.Namespace {
    ...
    init(uncheckedItemWindow itemWindow: WindowInfo, sourcePID: pid_t?) {
        ...
        {
            self = .string(Constants.bundleIdentifier)
