...
// MARK: - MenustowBarPanel

final class MenustowBarPanel: NSPanel {
    /// The shared app state.
    private weak var appState: AppState?

    /// Manager for the menustow Bar's color.
    private let colorManager = MenustowBarColorManager()
    ...
    /// Creates a new menustow Bar panel.
    init() {
        super.init(
            contentRect: .zero,
    ...
    private func updateOrigin(for screen: NSScreen) {
        ...
        func getOrigin(for menustowBarLocation: MenustowBarLocation) -> CGPoint {
            ...
            case .dynamic:
                ...
                return getOrigin(for: .menustowIcon)
            case .mousePointer:
        ...
        setFrameOrigin(getOrigin(for: appState.settings.general.menustowBarLocation))
    }
    ...
    func show(section: MenuBarSection.Name, on screen: NSScreen) async {
        ...
        appState.navigationState.isMenustowBarPresented = true
        ...
        do {
            ...
            Logger.default.error("Cache update failed when showing MenustowBarPanel - \(error)")
        }

        contentView = MenustowBarHostingView(
            appState: appState,
    ...
    override func close() {
        ...
        contentView = nil
        currentSection = nil
        appState?.navigationState.isMenustowBarPresented = false
    }
}

// MARK: - MenustowBarHostingView

private final class MenustowBarHostingView: NSHostingView<MenustowBarContentView> {
    override var safeAreaInsets: NSEdgeInsets { NSEdgeInsets() }

    init(
        appState: AppState,
        colorManager: MenustowBarColorManager,
        screen: NSScreen,
        section: MenuBarSection.Name
    ) {
        let rootView = MenustowBarContentView(
            appState: appState,
            colorManager: colorManager,
    ...
    @available(*, unavailable)
    required init(rootView: MenustowBarContentView) {
...
// MARK: - MenustowBarContentView

private struct MenustowBarContentView: View {
    @ObservedObject var appState: AppState
    @ObservedObject var colorManager: MenustowBarColorManager
    ...
    private var content: some View {
        if !ScreenCapture.cachedCheckPermissions() {
            HStack {
                Text("The menustow Bar requires screen recording permissions.")
            ...
            Text("menustow cannot display menu bar items for automatically hidden menu bars")
                .padding(.horizontal, 10)
...
// MARK: - MenustowBarItemView

private struct MenustowBarItemView: View {
    @ObservedObject var imageCache: MenuBarItemImageCache
    @ObservedObject var itemManager: MenuBarItemManager
    @ObservedObject var menuBarManager: MenuBarManager
...
// MARK: - MenustowBarItemClickView

private struct MenustowBarItemClickView: NSViewRepresentable {
